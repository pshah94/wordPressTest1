...
function so_current_register_scripts(){
	wp_register_script( 'so-current-flexslider' , get_template_directory_uri().'/js/jquery.flexslider.min.js' , array('jquery'), '2.1' );
	wp_register_script( 'so-current-fitvids' , get_template_directory_uri().'/js/jquery.fitvids.min.js' , array('jquery'), '1.0' );
}
...
function so_current_scripts() {
	...
	wp_enqueue_script( 'so-current-main' , get_template_directory_uri().'/js/jquery.theme-main.min.js' , array('jquery', 'so-current-flexslider', 'so-current-fitvids'), SITEORIGIN_THEME_VERSION );
 ...
 */
function current_viewport_header(){
	?><meta name="viewport" content="width=1080" /><?php
}
add_action('wp_head', 'current_viewport_header');